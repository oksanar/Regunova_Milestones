...
def filter_by_params(month: str, department: str, item: dict, by_field_name='date_of_birth'):
    date_time_parts = get_splitted_date(item[by_field_name])
    if date_time_parts['month'] == (MONTHS.index(month) + 1) and item['department'] == department:
        return item
    return None
...
def format_date(date: str):
    ...
    date_date = datetime(date_parts['year'], date_parts['month'], date_parts['day'])
    return date_date.strftime('%b %d').replace('0', '')


def get_employees_birthdays_by_department(filename: str, month_name: str, department: str):
    ...
    with open(filename, 'r') as file:
        ...
        for item in reader:
            ...
            if filtered_item:
                ...
                user_data = {'id': item['id'], 'name': item['name'], 'birthday': format_date(item['date_of_birth']), 'age': age}
                filtered_data['employees'].append(user_data)

    return filtered_data


def get_employees_anniversaries_by_department(filename: str, month_name: str, department: str):
    filtered_data = {'total': 0, 'employees': []}
...
def getBirthdays():
    ...
    print(department)
    result = get_employees_birthdays_by_department('database.csv', month, department)
...
def getAnniversaries():
    ...
    department = request.args.get('department')
    print(department)
    result = get_employees_anniversaries_by_department('database.csv', month, department)
    return Response(response=json.dumps(result),
                    status=200,
                    mimetype="application/json")
